...
exports.init = {
  call: async function (dbObj, data) {
    if (data && data.type == "list") {
      ...
      const condition = {
        select:
          "oi.*,  p.sku_no sku,p.name pname, p.code pcode, b.name branch_name",
        reference: [
          {
            type: "JOIN",
            obj: "product p",
      ...
      condition.select = "COUNT(oi.id) as total_count";
      delete condition.order;
      delete condition.limit;
      let itemsTot = await dbObj.getData("order_item oi", condition);
      ...
      if (items.SUCCESS && items.MESSAGE.length > 0) {
        ...
        return { SUCCESS: true, MESSAGE: atemArray, count : itemsTot.SUCCESS ? itemsTot.MESSAGE[0].total_count : 0, items: items };
      } else {
