    ...
    init : async function( rb){
      ...
      if(rb.TASK == 'location_import'){
        ...
        return new Promise(async function (result) {
          ...
          const res = await importData(commonObj, rb.type);
      ...
      }else if(rb.TASK == 'sales_import'){
        return new Promise(async function (result) {
          const importData = require('./se').init.importData;
          const res = await importData(commonObj);
          console.log('Product data import completed.');
          result(res);
        });
      }else if(rb.TASK == 'generate_order'){
        return new Promise(async function (result) {
          const generateOrder = require('./generate_order').init.generateOrder;
          const res = await generateOrder(commonObj);
          result(res);
        });
      }else if(rb.TASK == 'bdm_import'){
        return new Promise(async function (result) {
          const importData = require('./bdm').init.importData;
          const res = await importData(commonObj);
          result(res);
        });
      }
