...
class _ImageEditPageState extends State<ImageEditPage>
    ...
    try {
      ...
      if (byteData != null) {
        ...
        bool isWhite(int x, int y) {
          ...
          return pixels[offset] > 200 && pixels[offset + 1] > 200 && pixels[offset + 2] > 200; // luminance threshold
        }
  ...
  String _getUserFriendlyError(String rawError) {
    ...
    if (lower.contains('socket') || lower.contains('network') || lower.contains('connection') || lower.contains('failed host lookup')) {
    ...
    } else if (lower.contains('500') || lower.contains('502') || lower.contains('503') || lower.contains('server')) {
      return 'Server is temporarily unavailable. Please try again later.';
    } else if (lower.contains('exception') || lower.contains('error:') || rawError.length > 80) {
      return 'Failed to apply laminate layer. Please try again.';
    }
    return rawError;
  }
      ...
      listener: (context, state) {
        if (state.errorMessage != null) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(_getUserFriendlyError(state.errorMessage!)),
  ...
  bool _isPointInHorizontalOverlay(Offset localPos) {
    ...
    final double top = showHorizontalArrowAtTop
        ? _selection!.top - 62
        : _selection!.top + _selection!.height + 42;
  ...
  bool _isPointInVerticalOverlay(Offset localPos) {
    ...
    double left = showVerticalArrowAtLeft
        ? _selection!.left - 90
        : _selection!.left + _selection!.width + 42;
